...
def make_table(snowboards)
  ...
  renderer = TTY::Table::Renderer::ASCII.new(new_table)
  for i in 0...snowboards.length
    new_table << ["#{snowboards[i].id}", "#{snowboards[i].brand.capitalize}", "#{snowboards[i].model.capitalize}", "#{snowboards[i].year}", "$#{snowboards[i].price}", "#{snowboards[i].in_stock}"]
  end
...
# Ask the user for each bit of info and create a new snowboard instance
# Add to the record_count so that each user can get a unique id
# Return the new snowboard instance
...
# Start Program Logic Flow
# Initial table
puts ""
puts ""
puts "Todd's Snowboards (#{snowboards.length} total)"
...
until input == "q"
  ...
  if input == "c" # c is for create
    ...
    puts "snowboard deleted"
  elsif input == "q" # q is for quit
    puts "quit"
  else
    puts "I don't know that input please try again"
  end
  puts ""
  puts ""
  puts "Todd's Snowboards (#{snowboards.length} total)"
